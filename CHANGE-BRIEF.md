...
**2026-09-24 — Cardinal body color changed from warm red-orange to orange (after build).**
The Character concept above specifies a "warm red-orange" body. As built (`e0532f`), that color was nearly the same luminance as the spike red (`d24e42`) — contrast ratio 1.11:1, hue 12° vs. 5° — so in the spike-failure screenshot the bird was separated from the spikes mainly by its ink outline. That undercuts the spike-before-Landing-2 read, where the player must see the hazard next to the character.
The body fill is now orange `f07a2a` (1.53:1 against the spike, hue 24°). Lighter oranges separated further from the spikes but washed out the yellow beak, which carries the facing role (e.g. `f29a2e`: 1.92:1 vs. spike, 1.21:1 vs. beak); `f07a2a` keeps both near 1.5:1. Only the body fill changed: the crest stays in the original red (now reading as a contrasting accent), and the wings, beak, outline, shapes, and collider are unchanged. The Character concept text above is left as originally written; this entry supersedes its body-color line.
Playtesting should confirm the beak still reads clearly against the orange body at 1×.
