...
const record = {
  ...
  scope:'First Steps local control/retry slice + Cardinal / Two-Step Crossing extension', source_sha256:sorted,
  ...
  screenshots:['01-menu','02-failure','03-jump','04-complete','05-crossing'].map(n=>({file:'evidence/screens/'+n+'.png',sha256:hash(fs.readFileSync(path.join(evidence,'screens',n+'.png')))}))
};
